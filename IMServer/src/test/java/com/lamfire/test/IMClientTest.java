 ...
 */
public class IMClientTest implements MessageReceivedListener{

        public static void main(String[] args) {
            SnakeBuilder builder = new SnakeBuilder();
            builder.host("127.0.0.1");
            builder.port(9999);
            builder.messageReceivedListener(new IMClientTest());
            Snake snake = builder.build();
            snake.startup();

            SERVICE service = new SERVICE();
            service.setFrom("lamfire");
            service.setType("set");
            service.setNs("user.auth");

            ARGS params = new ARGS();
            params.put("userId","lamfire");
